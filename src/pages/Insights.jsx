...
export default function Insights() {
  ...
  const insights = useMemo(() => {
    ...
    const maxDay = Object.keys(dailyMap).length
  ...
  const metrics = [
    ...
    },
    {
      ...
      extra: (
        <div className="flex items-center gap-1 text-xs mt-1">
          ...
          <span
            className={insights.change > 0 ? "text-red-400" : "text-green-400"}
          >
            {insights.change}%
          </span>
        </div>
    ...
    { label: "Highest Spending Day", value: insights.maxDay, small: true },
    {
      label: "Savings Rate",
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-3 flex-1">
        {metrics.map((item, i) => (
          ...
          >
            ...
            <h3
              className={`${item.small ? "text-xs sm:text-sm" : "text-base sm:text-lg"} font-semibold ${item.color || "text-white"}`}
            >
              {item.value}
            </h3>
            {item.extra}
        ...
        <div className="col-span-1 sm:col-span-2 md:col-span-3 xl:col-span-4 bg-gradient-to-r from-purple-600/20 to-indigo-600/20 border border-white/10 rounded-xl p-3 text-xs sm:text-sm text-white flex items-center max-h-[80px]">
